...
[使用 Vultr 创建云主机详细步骤](https://www.playpi.org/2019072801.html) 
...
- `Affiliate`：**营销推广**模块，可以利用 `Vultr` 为你生成的唯一分享链接，拉取新用户注册使用，然后就会给你返优惠券，可抵扣消费，一般情况下用不到，不过可以分享给身边的人试试
...
支付方式就是用户选择使用什么方式来支付，除了常规的

## 查看历史消费记录

查看历史消费记录，就是为了对对账，看看有没有额外的乱扣费现象，一般情况下通过邮箱查看就行，不用特意登录 `Vultr` 里面看。

不过，账单默认是不会被发送到邮箱的，也不会通知用户，需要提前设置好通知方式，在 `Billing` 模块中的 `History` 标签页最底部有一个通知方式设置，选择通过邮箱即可，这个邮箱帐号是在 `Account` 模块中设置的，可以参考下文。

图。。

选择 `Billing` 模块中的 `Histroy` 标签页，可以看到我的消费记录，八月份扣除了我七月份的费用，总计 `$3.5`，这算是很便宜的主机配置。
...
其中还有一些是进账，或者是自己充值，或者是邀请新用户使用 `Vultr` 带来的代金券，可以看到在七月份有两笔钱进账，总计二十美元，可以抵扣我四个月的主机费用了。
