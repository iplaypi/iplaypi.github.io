...
此时，尝试本地调试运行 `ModuleARun.run()`【或者打 `jar` 包后运行】，可以发现如下错误：
...
看到 `NoSuchMethodError` 就知道出现了严重的问题，如果试图使用搜索功能搜索 `Strings` 这个类，可以发现有2个一模一样的类，但是他们对应的 `guava jar` 的版本号不一致。这时候有经验的工程师就可以立马判断，编译打包运行 `JVM` 加载的 `jar` 对于 `ModuleCRun.run()` 方法来说是有问题的，但是确保了 `ModuleBRun.run()` 方法可以顺利执行【和手动排除 `module-c` 中的 `guava v26.0-jre` 一个效果】。
...
除了演示c直接做shade，还要演示设置c-shade的方式。
...
因为源代码在编译阶段不会检测调用的是哪个依赖里面的类或者方法，必须是执行打包操作后才明确，但是打包过程对于重复的依赖无法确定具体是哪个依赖生效，打包完成后到运行的时候，就会出问题。注意这里虽然在 `module-b` 中对部分依赖做了 `shade`，但是只是对 `module-b` 生效，对 `module-a` 是无效的，所以可能会导致 `module-a` 中的代码引用时找不到类或者方法，编译打包正常，运行时会出现 `NoClassDefFoundError` 异常。
