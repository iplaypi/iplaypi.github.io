...
| 信号编号 | 信号名称 | 信号解释 |
| :------: | :------: | :------: |
| 1 | SIGHUP | 挂起【hang up】，终端断线，经常在退出系统前使用。但是，一般启动程序时为了让程序继续运行，会指定 nohup 就是为了不让程序挂起，这样在退出系统时程序仍旧能正常运行 |
...
| 12 | SIGUSR2 | 用户自定义信号2 |
| 18 | SIGCONT | 继续【与命令 fg/bg 对应】 |
| 19 | SIGSTOP | 暂停【与键盘快捷键 ctrl + z 对应】，可以使用信号18来继续运行，或者使用 fg/bg 来调度到前/后台继续运行 |
...
开启三个进程，分别使用 ctrl + z 命令暂停它们的运行，在暂停时输出的日志中会有 Stopped 标记，并且会有进程的编号分配，在方括号中的就是【有时候暂停时还会有**核心已转储**的提示】。
...
pid 就是系统给进程分配的编号，除了正常的正整数之外，其它的取值有各自特殊的含义。
