...
这里需要注意一点，如果把进程放在后台运行，由于进程不再占用会话窗口，它的本质其实是不再从标准输入【stdin】读取输入参数指令，如果此时进程中有从标准输入读取指令的代码逻辑，会导致报错退出。因此，对于一些交互式的任务，肯定不适合放在后台运行，况且本来就是交互式任务【与前台用户交互】，还放在后台运行干什么。如果非要放在后台运行，可以在执行任务时加上输入重定向【注意不是输出重定向】：`nohup command < /dev/null &`，这样就不会报错退出，但是也无法读取输入指令。【待测试，可以使用shell测试】

此外，如果想把输出信息输出到指定的文件，也可以使用 `>filename 2>&1` 来更改默认的重定向文件名，这样更方便观察每个进程的输出日志。
