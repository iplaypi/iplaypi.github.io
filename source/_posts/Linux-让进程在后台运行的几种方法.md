...
根据这个流程，首先可以明白为什么前台子进程会随着会话的关闭而终止，是因为前台子进程收到了会话发送的 `SIGHUP` 信号，不得不终止。
...
可以看到，在我的 `Linux` 系统中，这个参数的设置是 `off`，也就是会话在退出前不会把 `SIGHUP` 信号发送给后台子进程。
...
如果启动任务使用的是 `setsid command &` 命令，也不需要考虑输入流、输出流这个问题，因为 `setsid` 已经把启动进程的父进程变更为 `init` 进程，当退出会话后，输入流、输出流都会被重定向到 `init` 进程所属的流。【我猜测是 `/dev/console`、`/dev/null` 之类的设备，我测试发现，代码中如果有输入处理进程会卡住，有输出处理进程会正常把信息输出到当前会话窗口，如果关闭窗口则看不到输出，但是也不影响进程的正常运行，状态仍旧是 `running`，父进程变为 `init`，说明输出流在关闭会话后变更了。能力有限，这个问题的根本原因暂时存疑。】
...
使用上述步骤依次操作，可以发现 `nohup tail -f yy.log` 对应的进程已经不在了，尽管我手动给它设置了 `nohup`，也无济于事。而对于 `tail -f xx.log &` 对应的进程，我并没有手动设置 `nohup`，进程仍旧在运行，这里可以认定是操作系统的设置导致会话关闭时不会给后台子进程发送 `SIGHUP` 信号，当前父进程已经是 `init`，这个可以理解。
